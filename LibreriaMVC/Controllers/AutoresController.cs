...
namespace MyVet.Controllers
{
    ...
    public class AutoresController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> InsertAutor(InsertAutoresDto autoresDto)
        {
            var user = HttpContext.User;
            string token = user.Claims.FirstOrDefault(x => x.Type == TypeClaims.Token).Value;
            ResponseDto response = await _autoresServices.InsertAutor(token, autoresDto);
            return Ok(response);
        }

        [HttpPut]
        public async Task<IActionResult> UpdateAutor(InsertAutoresDto autoresDto)
        {
            var user = HttpContext.User;
            string token = user.Claims.FirstOrDefault(x => x.Type == TypeClaims.Token).Value;
            ResponseDto response = await _autoresServices.UpdateAutor(token,autoresDto);
            return Ok(response);
        }
