...
namespace MyVet.Controllers
{
    ...
    public class BooksController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> InsertBook(InsertBooksDto booksDto)
        {
            var user = HttpContext.User;
            string token = user.Claims.FirstOrDefault(x => x.Type == TypeClaims.Token).Value;
            ResponseDto response = await _booksServices.InsertBook(token, booksDto);
            return Ok(response);
        }

        [HttpPut]
        public async Task<IActionResult> UpdateBook(AlterBooksDto booksDto)
        {
            var user = HttpContext.User;
            string token = user.Claims.FirstOrDefault(x => x.Type == TypeClaims.Token).Value;
            ResponseDto response = await _booksServices.UpdateBook(token, booksDto);
            return Ok(response);
        }
