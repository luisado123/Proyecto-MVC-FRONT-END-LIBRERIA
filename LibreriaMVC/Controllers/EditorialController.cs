...
namespace MyVet.Controllers
{
    ...
    public class EditorialController : Controller
    {
        ...
        public async Task<IActionResult> InsertEditorial(EditorialDto editorialDto)
        {
            var user=HttpContext.User;
            string token = user.Claims.FirstOrDefault(x => x.Type == TypeClaims.Token).Value;
            ResponseDto response = await _editorialServices.InsertEditorial(token,editorialDto);
            return Ok(response);
        }
        [HttpPut]
        public async Task<IActionResult> UpdateEditorial(EditorialDto editorialDto)
        {
            var user = HttpContext.User;
            string token = user.Claims.FirstOrDefault(x => x.Type == TypeClaims.Token).Value;
            ResponseDto response = await _editorialServices.UpdateEditorial(token, editorialDto);
            return Ok(response);
        }
